...
def read_some_inventories_by(session: Session, by_: sa.Column, sort_: Callable, from_: int, to_: int):
    cte_query = (
        sa.select(sa.func.row_number().over(order_by=Inventory.id).label('current_id'),
    ...
    query = (
        sa.select(cte_query.c[
                      'inventory_id', 'product_name', 'product_description', 'product_price', 'location_name', 'inventory_quantity'])
...
def find_some_inventories_by(session: Session, q: str, by_: sa.Column, sort_: Callable, from_: int, to_: int):
    cte_query = (
        sa.select(sa.func.row_number().over(order_by=Inventory.id).label('current_id'),
                  Inventory.id.label('inventory_id'),
                  Product.name.label('product_name'),
...
def create_product(session: Session, product_name: str, product_description: str, product_price: float):
    session.add(Product(name=product_name, description=product_description, price=product_price))
